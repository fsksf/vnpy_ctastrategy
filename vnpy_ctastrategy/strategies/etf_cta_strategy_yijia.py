...
class ETFBSStrategyTempYiJia(ETFTemplate):
    ...
    def handle_bar(self, bar: BarData):
        ...
        """
        self.cancel_all()
        if self.basket_pos < 1 and self.etf_pos < 2000000:
            print('买入ETF')
            self.buy_sell_with_target(limit_price=3, target_volume=2000000, per_order_max=1000000)

        elif self.etf_pos >= 2000000:
            print('赎回')
            self.redemption(2000000)
